...
// a non-numpy alternative for `detail::get_2darray()`.
  ...
  // add by weihao 2024-4-24
  template<typename Numeric>
  void plot3(const std::vector<Numeric> &x,
                  const std::vector<Numeric> &y,
                  const std::vector<Numeric> &z,
                  const std::vector<Numeric> &s_x,
                  const std::vector<Numeric> &s_y,
                  const std::vector<Numeric> &s_z,
                  const std::map<std::string, std::string> &keywords,
                  const std::map<std::string, std::string> &keywords_smooth,
                  const long fig_number=0) {
    detail::_interpreter::get();
    ...
    PyObject *sxarray = detail::get_array(s_x);
    PyObject *syarray = detail::get_array(s_y);
    PyObject *szarray = detail::get_array(s_z);
    ...
    PyObject *smooth_args = PyTuple_New(3);
    PyTuple_SetItem(smooth_args, 0, sxarray);
    PyTuple_SetItem(smooth_args, 1, syarray);
    PyTuple_SetItem(smooth_args, 2, szarray);

    // Build up the kw args.
    PyObject *kwargs = PyDict_New();
    PyObject *smooth_kwargs = PyDict_New();
    ...
    for (std::map<std::string, std::string>::const_iterator it = keywords_smooth.begin();
         it != keywords_smooth.end(); ++it) {
      PyDict_SetItemString(smooth_kwargs, it->first.c_str(),
                           PyString_FromString(it->second.c_str()));
    }
    ...
    Py_INCREF(plot3);
    PyObject *res = PyObject_Call(plot3, args, kwargs);
    PyObject *res_smooth = PyObject_Call(plot3, smooth_args, smooth_kwargs);
    if (!res) throw std::runtime_error("Failed 3D line plot");
    Py_DECREF(plot3);
    ...
    Py_DECREF(kwargs);
    if (res) Py_DECREF(res);
    if (res_smooth) Py_DECREF(res_smooth);
  }
  template<typename Numeric>
